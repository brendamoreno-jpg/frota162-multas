    ...
    >
      ...
      >
        <div style={{ padding: '10px 8px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          <MenuItem icon={<DetIconDownload />}  label="Baixar boleto 20%"                  onClick={onClose} />
          <MenuItem icon={<DetIconRefresh />}   label="Atualizar boleto 20%"               onClick={onClose} />
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 6, flexShrink: 0, background: 'var(--color-primary-200)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-primary-600)' }}><DetIconRefresh /></div>
                    <span style={{ fontSize: 18, fontWeight: 700, color: 'var(--color-neutral-900)' }}>Multas relacionadas</span>
                  </div>
                  <div style={{ height: 1, background: 'var(--color-neutral-300)' }} />
                </div>
                {ait.multasRelacionadas && ait.multasRelacionadas.length > 0 ? (
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    {/* Header da tabela */}
                    <div style={{ display: 'flex', padding: '8px 16px', background: 'var(--color-neutral-100)', borderBottom: '1px solid var(--color-neutral-300)' }}>
                      {['AIT', 'Infração', 'Valor da Multa', 'Com Desconto', 'Valor Pago', 'Vencimento'].map(col => (
                        <span key={col} style={{ flex: 1, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--color-neutral-700)' }}>{col}</span>
                      ))}
                    </div>
                    {ait.multasRelacionadas.map((m, i) => (
                      <div key={i} style={{ display: 'flex', padding: '12px 16px', borderBottom: '1px solid var(--color-neutral-200)', alignItems: 'center' }}>
                        <span style={{ flex: 1, fontSize: 13, fontWeight: 700, color: 'var(--color-primary-600)', display: 'inline-flex', alignItems: 'center', gap: 4 }}>{m.ait} <DetIconLinkOut /></span>
                        <span style={{ flex: 1, fontSize: 13, color: 'var(--color-neutral-900)' }}>{m.infracao}</span>
                        <span style={{ flex: 1, fontSize: 13, color: 'var(--color-neutral-900)' }}>{m.valor}</span>
                        <span style={{ flex: 1, fontSize: 13, color: 'var(--color-neutral-900)' }}>{m.valorDesconto}</span>
                        <span style={{ flex: 1, fontSize: 13, color: 'var(--color-neutral-900)' }}>{m.valorPago || '—'}</span>
                        <span style={{ flex: 1, fontSize: 13, color: 'var(--color-neutral-900)' }}>{m.vencimento}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 16px', color: 'var(--color-neutral-400)', fontSize: 14 }}>
                    Nenhuma multa relacionada
                  </div>
                )}
              </div>
